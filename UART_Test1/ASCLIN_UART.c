...
#define UART_PIN_TX             IfxAsclin0_TX_P15_3_OUT
...
{
    cnt_led = 0;
    for(int i=MAX_RCV_BUFFER_SIZE-4; i>=0; i--)
    {
        if(g_rxRcvBuffer[i+0] == '#' && g_rxRcvBuffer[i+1] == 'c' && g_rxRcvBuffer[i+3] == '*')
        {
            cnt_led = g_rxRcvBuffer[2];
            return 1;
        }
    }
    return 0;
