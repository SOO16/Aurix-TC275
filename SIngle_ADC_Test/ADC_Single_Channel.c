...
MovingAverage g_adcMovingAverage;
...
/*Average 값으로 노이즈 감소*/
void initMovingAverage(MovingAverage *ma, uint16 windowSize)
{
    ma->average = 0.0;
    ma->windowSize = windowSize;
    ma->sampleCount = 0;
}
float updateMovingAverage(MovingAverage *ma, uint16 new_sample)
{
    if(ma->sampleCount < ma->windowSize)
    {
        ma->average = (ma->average * ma->sampleCount + (float)new_sample / (ma->sampleCount + 1));
        ma->sampleCount++;
    }
    else
    {
        ma->average = ma->average + ((float)new_sample - ma->average) / ma->windowSize;
    }
}

uint16 getSmoothedValue(MovingAverage *ma)
{
    return (unsigned short)(ma->average + 0.5f);
}
