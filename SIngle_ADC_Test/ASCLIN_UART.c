...
void asclin0RxISR(void)
{
    ...
    flag_enable_adc = decode_adc_command();
}
...
int decode_adc_command(void)
{
    adc_group = 0;
    adc_channel = 0;
    for(int i=MAX_RCV_BUFFER_SIZE-6; i>=0; i--)
    {
        if((g_rxRcvBuffer[i+0] == '#') && (g_rxRcvBuffer[i+1] == 'A') && (g_rxRcvBuffer[i+5] == '*'))
        {
            adc_group = g_rxRcvBuffer[i+2];
            adc_channel = g_rxRcvBuffer[i+3];
            if(g_rxRcvBuffer[i+2] == 'E') return 1;
            if(g_rxRcvBuffer[i+2] == '0') return 0;
        }
    }
    return -1;
}
